...
export default function ResetPassword() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'An error occurred. The token may be expired.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="auth-root" style={{
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <h2 style={{ fontSize: 26, fontWeight: 800, color: '#fff', letterSpacing: '-.5px', marginBottom: 6 }}>Reset Password</h2>
          ...
          {error ? (
            <div style={{ background: 'rgba(239,68,68,.15)', border: '1px solid rgba(239,68,68,.35)', color: '#fca5a5', borderRadius: 16, padding: '16px', fontSize: 13, textAlign: 'center', fontWeight: 500, marginBottom: 20 }}>
              {error}
              <div style={{ marginTop: 12 }}>
                <Link to="/forgot-password" style={{ color: '#fff', textDecoration: 'underline' }}>Request a new link</Link>
              </div>
            </div>
          ) : (
